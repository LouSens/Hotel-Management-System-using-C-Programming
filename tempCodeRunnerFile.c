...
void bookAccommodation(const char *currentUser) {
    ...
    // Reopen accommodations.txt in read+write mode
    accFile = fopen("accommodations.txt", "r");
    if (!accFile) {
        printf("Error reopening accommodations file!\n");
        waitForEnter();
        return;
    }
    ...
    scanf("%s", type); // Ensure proper reading of input here
    ...
    while (fscanf(accFile, "%s %d %f %d %d", acc.type, &acc.capacity, &acc.price, &acc.totalRooms, &acc.availableRooms) == 5) {
        if (strcmp(type, acc.type) == 0) {
            ...
            if (acc.availableRooms > 0) {
                ...
                // Write booking details to bills.txt
                ...
                printf("No available rooms for %s! Please choose another accommodation type.\n", acc.type);
                fprintf(tempFile, "%s %d %.2f %d %d\n", acc.type, acc.capacity, acc.price, acc.totalRooms, acc.availableRooms);
            }
        } else {
            fprintf(tempFile, "%s %d %.2f %d %d\n", acc.type, acc.capacity, acc.price, acc.totalRooms, acc.availableRooms);
...
void viewBill(const char *username) {
    char fileUsername[50], fileName[50], customerEmail[50], customerPhone[15];
    char billDetails[200];
    char name[50] = "Customer"; // Default name if not found
    FILE *infoFile = fopen("customer_info.txt", "r");
    FILE *billFile = fopen("bills.txt", "r");
    int found = 0;

    if (infoFile == NULL || billFile == NULL) {
        printf("Error opening files.\n");
        return;
    }

    // Look for the name in customer_info.txt
    while (fscanf(infoFile, "%s %s %s", fileUsername, fileName, customerEmail) != EOF) {
        if (strcmp(username, fileUsername) == 0) {
            strcpy(name, fileName); // Retrieve the name
            break;
        }
    }
    fclose(infoFile);
    ...
    clearScreen();
    printf("Viewing bills for %s:\n", name); // Use the retrieved name
    printf("---------------------------");
    // Look for the bills in bills.txt
    while (fscanf(billFile, "%s %[^\n]", fileUsername, billDetails) != EOF) {
        if (strcmp(username, fileUsername) == 0) {
            printf("%s\n", billDetails);
            found = 1;
        }
